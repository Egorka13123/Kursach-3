...
st.subheader('Быковский Егор, Фпэ-01-19, Вариант 4')
st.caption('Ne = 214 МВт')
st.caption('p0 = 12.7 МПа')
st.caption('t0 = 544 C')
st.caption('ppp = 2-3 МПа')
st.caption('tpp = 546 C')
st.caption('pk = 4,4 кПа')
st.caption('tpv = 234 C')
st.caption('z = 9 шт.')
...
itog=pd.DataFrame({
"eta_ir": (eta),
"Давление": (list(np.arange(2, 3.1, 0.1))),
"G_0, кг/с": (G0),
"G_k, кг/с": (Gk)
})
...
p = figure(
     title='Зависимость КПД от давления',
     x_axis_label='Давление',
     y_axis_label='КПД')

p.line(x, y, legend_label='Зависимость КПД от давления', line_width=3)
...
p_pp=st.slider('p_pp',2.0,3.0,2.0,0.1)
